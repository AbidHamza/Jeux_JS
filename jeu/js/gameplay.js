...
function interaction(data,im,bign){
    if (data.type==3){
        genereObjet(data);
    }
    if (data.type==7){
    ...
    if (data.type==5){
        devPorte(data,im,bign);
    }
    if (data.type==6){
        fermeturePorte(data,im,bign);
    }
    if (data.type==9){
        ouverturePorte(data,im,bign);
    }
    if (data.type==4){
        dbox("Vous avez réussi à atteindre l'escalier de l'étage, mais vous ignorez ce qui vous attends plus bas...")
        var element = document.getElementById("lvl-select");
        element.value=2
...
function ouverturePorte(data,ime,bign){
    console.log(data)
    var choix = confirm("Voulez-vous ouvrir la porte?")
    if (choix){
        data.type = 6
        let n = genereNode(bign)
        graphe(n)
        console.log(bign,graph)
...
function fermeturePorte(data,ime,bign){
    console.log("D"+ime)
...
function devPorte(data,ime,bign){
    for(i=1;i<11;i++) {
    ...
    return dbox("Vous n'avez pas la clé pour ouvrir cette porte ! ")
